...
describe('ws subscribe handler', () => {
    beforeEach(() => {
        ...
        mockStorage.subscribe.mockResolvedValue(undefined);
    });

    it('subscribes deduped non-empty subjects', async () => {
        ...
        expect(mockStorage.subscribe).toHaveBeenCalledTimes(2);
        expect(mockStorage.subscribe).toHaveBeenCalledWith('conn-1', 'user-1', 'case', '22CR1');
        expect(mockStorage.subscribe).toHaveBeenCalledWith('conn-1', 'user-1', 'case', '22CR2');
