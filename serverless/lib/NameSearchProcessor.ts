            ...
            if (portalCredentials) {
                try {
                    ...
                    if (!authResult.success || !authResult.cookieJar) {
                        ...
                        // Return empty results - the client will still have the searchId
                        // and can poll for updates
                    } else {
                ...
                console.log(`No portal credentials found for user ${userId}`);
                // Return empty results - the client will still have the searchId
                // and can poll for updates
            }
        }

        // Return the search ID and empty results for now
        // Client will poll for updates using the search ID
        return {
            searchId,
            results: {},
        };
    } catch (error) {
        console.error('Error processing name search request:', error);
        // Still return the searchId if we generated one, so the client can poll
        return {
            searchId: searchId || '',
            results: {},
        };
