const SearchParser = {
    parseSearchInput(input: string): string[] {
        ...
        // Example: 5902022S 714844 => 22CR714844-590 (superior court; assume CR)
        const normalized = input.replace(
            /(?<county_code>\d{3})(?:19|20)(?<year>\d{2})(?:(?<case_type>[A-Za-z]{2,3})|S)(?:\s\n?)?(?<case_no>\d{6})/g,
            (_match, ...args) => {
        ...
        const caseNumbers = [];
        const regex = /\d{2}[A-Za-z]{2}[ A-Za-z]?\d{6}-\d{3}/g;
        let match;

        while ((match = regex.exec(normalized)) !== null) {
            caseNumbers.push(match[0].replace(/\s/g, ''));
        }
